...
int main(int ac, char **av) {
    ...
    while (1) {
        ...
        if (select(max + 1, &read_fds, &write_fds, NULL, NULL) < 1) continue;
        for (int fd = 0; fd <= max; fd++) {
            if (FD_ISSET(fd, &read_fds) && fd == sockfd) {
                int Clt = accept(sockfd, NULL, NULL);
                max = Clt > max ? Clt : max;
                clients[Clt].id = next_id++;
                bzero(clients[Clt].msg, strlen(clients[Clt].msg));
                FD_SET(Clt, &active_fds);
